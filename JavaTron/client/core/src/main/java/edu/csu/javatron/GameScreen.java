...
public class GameScreen extends ScreenAdapter {
    ...
    public void updateTrails() {
        ...
        if (game.roundNumber != prevRoundNumber) {
            ...
            prevAx = game.ax; prevAy = game.ay;
            prevBx = game.bx; prevBy = game.by;
            return;
        }
    ...
    public void render(float delta) {
        ...
        Color pColor = getColorFromString(game.playerColor, Color.BLUE);
        Color oColor = getColorFromString(game.oppColor, Color.RED);
        ...
        font.setColor(pColor);
        font.draw(spriteBatch, game.playerName + ": " + game.aWins, tx, ty - 44);
        font.setColor(Color.WHITE);
        font.draw(spriteBatch, "vs", tx, ty - 66);
        font.setColor(oColor);
        font.draw(spriteBatch, game.oppName + ": " + game.bWins, tx, ty - 88);
        ...
        if (msg != null) {
            if ("GO".equals(msg)) {
                ...
                if (goDisplayTimer <= 0f) goDisplayTimer = 1.5f;
        ...
        boolean upPressed    = Gdx.input.isKeyJustPressed(game.upKey)    || Gdx.input.isKeyJustPressed(com.badlogic.gdx.Input.Keys.UP);
        boolean downPressed  = Gdx.input.isKeyJustPressed(game.downKey)  || Gdx.input.isKeyJustPressed(com.badlogic.gdx.Input.Keys.DOWN);
        boolean leftPressed  = Gdx.input.isKeyJustPressed(game.leftKey)  || Gdx.input.isKeyJustPressed(com.badlogic.gdx.Input.Keys.LEFT);
        boolean rightPressed = Gdx.input.isKeyJustPressed(game.rightKey) || Gdx.input.isKeyJustPressed(com.badlogic.gdx.Input.Keys.RIGHT);

        String turnDir = null;
        if (upPressed)         turnDir = "D"; // W/↑ → send 'D' which is +y = up on screen
        else if (downPressed)  turnDir = "U"; // S/↓ → send 'U' which is -y = down on screen
        else if (leftPressed)  turnDir = "L";
        else if (rightPressed) turnDir = "R";

        if (turnDir != null) {
            game.getNetworkClient().send("C_TURN|" + turnDir);
            if (turnSound != null && SettingsScreen.isAudioEnabled()) turnSound.play(0.5f);
        }
    }
