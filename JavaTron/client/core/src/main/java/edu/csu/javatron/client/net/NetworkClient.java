...
public class NetworkClient {
    ...
    private void handleMessage(String message) {
        ...
        if (message.startsWith(Protocol.S_SNAPSHOT)) {
            ...
            for (String part : parts) {
                if (part.startsWith("oppName=")) game.oppName = part.substring(8);
                else if (part.startsWith("oppColor=")) game.oppColor = part.substring(9);
            }
            Gdx.app.postRunnable(() -> game.showGameScreen());
        } else if (message.startsWith(Protocol.S_MATCH_START)) {
            // New match or rematch started by server - reset countdown state and transition to game
    ...
    private void parseSnapshot(String message) {
        ...
        long tick = 0;
        for (String part : parts) {
            if (part.startsWith("tick=")) tick = Long.parseLong(part.substring(5));
            else if (part.startsWith("ax=")) ax = Integer.parseInt(part.substring(3));
            else if (part.startsWith("ay=")) ay = Integer.parseInt(part.substring(3));
            else if (part.startsWith("bx=")) bx = Integer.parseInt(part.substring(3));
            else if (part.startsWith("by=")) by = Integer.parseInt(part.substring(3));
            else if (part.startsWith("adir=")) aDir = part.charAt(5);
            else if (part.startsWith("bdir=")) bDir = part.charAt(5);
            ...
            } else if (part.startsWith("round=")) roundNumber = Integer.parseInt(part.substring(6));
        }
