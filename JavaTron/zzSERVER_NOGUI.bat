...
set "LOGFILE=%~dp0logs\zzSERVER_NOGUI_%ts%.log"

echo Writing log to: "%LOGFILE%"
echo ==== zzSERVER_NOGUI ====>> "%LOGFILE%"
echo StartTime: %DATE% %TIME%>> "%LOGFILE%"
echo WorkingDir: %CD%>> "%LOGFILE%"
echo Command: call .\gradlew.bat :server:run --args="--nogui" --console=plain --info --stacktrace>> "%LOGFILE%"
echo ---- output ---->> "%LOGFILE%"

call .\gradlew.bat :server:run --args="--nogui" --console=plain --info --stacktrace >> "%LOGFILE%" 2>&1
